...
spark.read.json(input_currencies)\
    .withColumn("date", col("end_at").cast("date")) \
    .withColumn("conversion_rate", col("rates.{target_date}.{target_currency}".format(target_date=target_date, target_currency=target_currency)))\
    .withColumnRenamed("base", "from")\
    .withColumn("to", lit(target_currency))\
    .drop("rates", "start_at", "end_at")\
